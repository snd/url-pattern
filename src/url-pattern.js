...
;(function(root, factory) {
  ...
  const concatMap = function(array, f) {
    let results = []
    let i = -1
    const {length} = array
    while (++i < length) {
  ...
  const stringConcatMap = function(array, f) {
    let result = ''
    let i = -1
    const {length} = array
    while (++i < length) {
  ...
  const regexGroupCount = regex =>
    new RegExp(regex.toString() + '|').exec('').length - 1

  const keysAndValuesToObject = function(keys, values) {
    const object = {}
    let i = -1
    const {length} = keys
    while (++i < length) {
  ...
  P.tag = (tag, parser) =>
    function(input) {
      ...
      const tagged = new P.Tagged(tag, result.value)
  ...
  P.pick = (indexes, ...parsers) =>
    function(input) {
      const result = P.sequence(...Array.from(parsers || []))(input)
      if (result == null) {
        return
      }
  ...
  P.concatMany1Till = (parser, end) => input =>
  ...
  const newParser = function(options) {
    const U = {}
    ...
    U.named = P.tag(
      'named',
      P.pick(1, P.string(options.segmentNameStartChar), P.lazy(() => U.name))
    )
    ...
    U.static = P.tag(
      'static',
      P.concatMany1Till(
        P.firstChoice(P.lazy(() => U.escapedChar), P.regex(/^./)),
        P.firstChoice(
    ...
    U.token = P.lazy(() =>
      P.firstChoice(U.wildcard, U.optional, U.named, U.static)
    )
  ...
  const astNodeToRegexString = function(astNode, segmentValueCharset) {
  ...
  const getParam = function(params, key, nextIndexes, sideEffects) {
    if (sideEffects == null) {
      sideEffects = false
    }
  ...
  var astNodeContainsSegmentsForProvidedParams = function(
    astNode,
    params,
    nextIndexes
  ) {
  ...
  var UrlPattern = function(arg1, arg2) {
    ...
    if (this.isRegex) {
      ...
      if (arg2 != null) {
        ...
        if (arg2.length !== groupCount) {
          throw new Error(
            `regex contains ${groupCount} groups but array of group names contains ${
              arg2.length
            }`
          )
        }
    ...
    const options = {
      escapeChar:
        (arg2 != null ? arg2.escapeChar : undefined) ||
        defaultOptions.escapeChar,
      segmentNameStartChar:
        (arg2 != null ? arg2.segmentNameStartChar : undefined) ||
        defaultOptions.segmentNameStartChar,
      segmentNameCharset:
        (arg2 != null ? arg2.segmentNameCharset : undefined) ||
        defaultOptions.segmentNameCharset,
      segmentValueCharset:
        (arg2 != null ? arg2.segmentValueCharset : undefined) ||
        defaultOptions.segmentValueCharset,
      optionalSegmentStartChar:
        (arg2 != null ? arg2.optionalSegmentStartChar : undefined) ||
        defaultOptions.optionalSegmentStartChar,
      optionalSegmentEndChar:
        (arg2 != null ? arg2.optionalSegmentEndChar : undefined) ||
        defaultOptions.optionalSegmentEndChar,
      wildcardChar:
        (arg2 != null ? arg2.wildcardChar : undefined) ||
        defaultOptions.wildcardChar
    }

    const parser = newParser(options)
